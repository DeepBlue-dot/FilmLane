...
export default function TvDiscoveryPage() {
  ...
  useEffect(() => {
    const fetchMetadata = async () => {
      try {
        ...
        console.error('Error fetching TV metadata', err);
      }
    };
    fetchMetadata();
  }, []);
  ...
  // Fetch TV Shows on filter/page change
  useEffect(() => {
    const fetchShows = async () => {
      ...
      try {
        ...
        if (selectedYear) {
          params.first_air_date_year = parseInt(selectedYear, 10);
        }
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 border-b border-gray-900 pb-5">
        <div>
          <h1 className="text-3xl font-extrabold text-white tracking-tight">Explore TV Shows</h1>
          <p className="text-gray-400 mt-1 text-sm">Browse, filter, and discover your next favorite series.</p>
        </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-16 text-gray-500">
          <p className="text-lg font-semibold text-gray-400">No TV shows found</p>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-6">
            {shows.map((show) => (
              <MediaCard
                ...
                item={show}
                type="tv"
                isBookmarked={watchlistIds.includes(show.id)}
